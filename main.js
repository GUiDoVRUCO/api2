...
const { exec } = require('child_process'); // Adicionado para executar o comando do VS Code
...
async function isAdmin(sender, username, password) {
  const user = await User.findOne({ username });
  if (user && await bcrypt.compare(password, user.password)) {
    return sender === ADMIN_PHONE; // Apenas o número do admin pode gerenciar usuários
  }
  return false;
}

// Funções de gerenciamento de usuários (adaptadas do user-control-app)
async function listarUsuarios(sender) {
  const users = await AppUser.find();
  if (users.length === 0) {
    return "Nenhum usuário encontrado.\n\n" + mostrarMenu();
  }
  const message = users.map(u => `ID: ${u._id}, Usuário: ${u.username}, Pode Instalar: ${u.canInstall ? 'Sim' : 'Não'}, Criado: ${u.createdAt}`).join('\n');
  return `Lista de Usuários:\n${message}\n\n${mostrarMenu()}`;
}

async function adicionarUsuario(sender, username, canInstall) {
  ...
  await user.save();
  logger.info(`Usuário adicionado: ${username}, ${sender}`);
  await notificarAdmin(client, `Novo usuário: ${username}, Pode Instalar: ${canInstall === '1' ? 'Sim' : 'Não'}`);
  return `Usuário ${username} adicionado com sucesso!\n\n${mostrarMenu()}`;
}

async function atualizarUsuario(sender, id, canInstall) {
  const result = await AppUser.updateOne(
    { _id: id },
    { canInstall: canInstall === '1' }
  );
  if (result.modifiedCount === 0) {
    return "Usuário não encontrado ou sem alterações.\n\n" + mostrarMenu();
  }
  logger.info(`Usuário atualizado: ID ${id}, ${sender}`);
  await notificarAdmin(client, `Usuário ID ${id} atualizado: Pode Instalar: ${canInstall === '1' ? 'Sim' : 'Não'}`);
  return `Permissão atualizada com sucesso!\n\n${mostrarMenu()}`;
}

async function deletarUsuario(sender, id) {
  const result = await AppUser.deleteOne({ _id: id });
  if (result.deletedCount === 0) {
    return "Usuário não encontrado.\n\n" + mostrarMenu();
  }
  logger.info(`Usuário deletado: ID ${id}, ${sender}`);
  await notificarAdmin(client, `Usuário ID ${id} deletado.`);
  return `Usuário deletado com sucesso!\n\n${mostrarMenu()}`;
}

// Funções existentes do bot
async function listarHorarios(sender) {
...
async function processarMensagem(mensagem, sender, client) {
  ...
  if (!estado.etapa && !["1", "2", "3", "4", "5", "6", "7", "8"].includes(mensagem) && !mensagemLower.startsWith("triagem") && !mensagemLower.startsWith("feedback") && !mensagemLower.startsWith("relatorio")) {
    return mostrarMenu();
  }
  ...
  if (mensagem === "7") {
    if (sender !== ADMIN_PHONE) {
      return "Apenas o administrador pode gerenciar usuários.\n\n" + mostrarMenu();
    }
    estado.etapa = "admin_login";
    return "Digite suas credenciais (ex.: gui12 admin123):";
  } else if (estado.etapa === "admin_login") {
    const [username, password] = mensagem.split(" ");
    if (await isAdmin(sender, username, password)) {
      estado.etapa = "gerenciar_usuarios";
      return "Gerenciar Usuários:\n1. Listar usuários\n2. Adicionar usuário\n3. Atualizar usuário\n4. Deletar usuário\n5. Voltar ao menu";
    }
    return "Credenciais inválidas. Tente novamente ou 'sair' para voltar ao menu:";
  } else if (estado.etapa === "gerenciar_usuarios") {
    if (mensagem === "1") {
      return await listarUsuarios(sender);
    } else if (mensagem === "2") {
      estado.etapa = "adicionar_usuario";
      return "Digite o nome do usuário e permissão (ex.: teste 1) ou 'sair':";
    } else if (mensagem === "3") {
      estado.etapa = "atualizar_usuario";
      return "Digite o ID do usuário e nova permissão (ex.: 123 1) ou 'sair':";
    } else if (mensagem === "4") {
      estado.etapa = "deletar_usuario";
      return "Digite o ID do usuário (ex.: 123) ou 'sair':";
    } else if (mensagem === "5") {
      estados.delete(sender);
      return mostrarMenu();
    }
    return "Opção inválida. Escolha: 1, 2, 3, 4 ou 5.";
  } else if (estado.etapa === "adicionar_usuario") {
    const [username, canInstall] = mensagem.split(" ");
    if (!username || !["0", "1"].includes(canInstall)) {
      return "Formato: [nome] [0 ou 1] (ex.: teste 1) ou 'sair':";
    }
    estados.delete(sender);
    return await adicionarUsuario(sender, username, canInstall);
  } else if (estado.etapa === "atualizar_usuario") {
    const [id, canInstall] = mensagem.split(" ");
    if (!id || !["0", "1"].includes(canInstall)) {
      return "Formato: [ID] [0 ou 1] (ex.: 123 1) ou 'sair':";
    }
    estados.delete(sender);
    return await atualizarUsuario(sender, id, canInstall);
  } else if (estado.etapa === "deletar_usuario") {
    if (!mensagem.match(/^[0-9a-fA-F]{24}$/)) {
      return "ID inválido. Digite o ID do usuário (ex.: 123) ou 'sair':";
    }
    estados.delete(sender);
    return await deletarUsuario(sender, mensagem);
  }
...
client.on('qr', async qr => {
  // Salvar o QR code como uma imagem no arquivo qrcode.png
  qrcode.toFile('qrcode.png', qr, { type: 'png' }, (err) => {
    if (err) {
      logger.error('Erro ao gerar QR code:', err);
      return;
    }
    logger.info('QR Code gerado e salvo como qrcode.png!');
    console.log('QR Code gerado e salvo como qrcode.png!');

    // Abrir o arquivo qrcode.png no VS Code
    exec('code qrcode.png', (err) => {
      if (err) {
        logger.error('Erro ao abrir o QR code no VS Code:', err);
        console.error('Erro ao abrir o QR code no VS Code:', err);
        return;
      }
      logger.info('QR Code aberto no VS Code! Escaneie com o WhatsApp.');
      console.log('QR Code aberto no VS Code! Escaneie com o WhatsApp.');
    });
  });

  // Manter a funcionalidade original de armazenar o QR code para a API
...
client.on('message', async msg => {
  ...
  const resposta = await processarMensagem(mensagem, sender, client);
  if (resposta) {
